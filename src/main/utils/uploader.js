...
class Uploader {
  constructor (img, webContents, sealgo = undefined) {
    this.img = img
  ...
  upload () {
    ...
    const sealgo = this.sealgo || new SealGo(CONFIG_PATH)
    sealgo.config.debug = true
    // for sealgo-core
    sealgo.config.SEALGO_ENV = 'GUI'
    let input = this.img

    sealgo.helper.beforeUploadPlugins.register('renameFn', {
      handle: async ctx => {
        const rename = sealgo.getConfig('settings.rename')
        const autoRename = sealgo.getConfig('settings.autoRename')
    ...
    sealgo.on('notification', message => {
      const notification = new Notification(message)
      notification.show()
    })

    sealgo.on('uploadProgress', progress => {
      this.webContents.send('uploadProgress', progress)
    })

    return new Promise((resolve) => {
      sealgo.on('finished', ctx => {
