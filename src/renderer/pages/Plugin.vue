<template>
  <div id="plugin-view">
    ...
    <el-row class="handle-bar" :class="{ 'cut-width': pluginList.length > 6 }">
      <el-input
        v-model="searchText"
        placeholder="搜索npm上的SealGo插件，或者点击上方按钮查看优秀插件列表"
        size="small"
      >
...
export default {
  ...
  computed: {
    npmSearchText () {
      return this.searchText.match('sealgo-plugin-')
        ? this.searchText
        : this.searchText !== ''
          ? `sealgo-plugin-${this.searchText}`
          : this.searchText
    }
  },
  watch: {
  ...
  methods: {
    buildContextMenu (plugin) {
      ...
      let menu = [{
        ...
        click () {
          _this.$db.read().set(`sealgoPlugins.sealgo-plugin-${plugin.name}`, true).write()
          plugin.enabled = true
          _this.getPicBeds()
        }
      }, {
        ...
        enabled: plugin.enabled,
        click () {
          _this.$db.read().set(`sealgoPlugins.sealgo-plugin-${plugin.name}`, false).write()
    ...
    async handleConfirmConfig () {
      ...
      if (result !== false) {
        switch (this.currentType) {
          case 'plugin':
            this.$db.read().set(`sealgo-plugin-${this.configName}`, result).write()
            break
          case 'uploader':
    ...
    handleSearchResult (item) {
      const name = item.package.name.replace(/sealgo-plugin-/, '')
      let gui = false
      if (item.package.keywords && item.package.keywords.length > 0) {
        if (item.package.keywords.includes('sealgo-gui-plugin')) {
          gui = true
        }
      }
      return {
        ...
        hasInstall: this.pluginNameList.some(plugin => plugin === item.package.name.replace(/sealgo-plugin-/, '')),
    ...
    goAwesomeList () {
      this.$electron.remote.shell.openExternal('https://github.com/foryourfuture/Awesome-SealGo')
    }
  },
  beforeDestroy () {
